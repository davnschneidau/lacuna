# Lacuna — Architecture & Build Plan
...
│   ├── harness.py                 # Sets up workspace, clones repos, invokes Claude Code
│   ├── kg/
│   │   ├── schema.sql
│   │   ├── client.py
│   │   └── primitives.py
...
These are the deterministic backbone. Agents call these instead of `grep` so they don't have to ingest raw output. Each MCP server is implemented as a Python process; the harness starts them and registers them with Claude Code via `.mcp.json`.
...
## 18. Build order (what to build first)

This is the order I'd actually do it in. Each stage is shippable on its own.

1. **KG + schema + Python client.** Nothing reasons until storage exists.
2. **`lacuna-recon` MCP server with a handful of tools** (`app_inventory`, `file_tree`, `entrypoints`, `dependency_graph`, `taint_paths`, `code_excerpt`). Get the deterministic foundation under you before any agent.
3. **Harness + Dockerfile.** Can you `docker run` it against a sample manifest and see Recon write to the KG? Ship that first.
4. **Orchestrator CLAUDE.md + `recon` subagent + Stop hook.** Minimum viable loop: orchestrator runs Recon, KG has an Application Model.
5. **One hunter end-to-end** (`hunter-injection`). One validator. PostToolUse evidence capture. Watch a finding land in the KG.
6. **Primitive extraction + chain-builder.** Even with one hunter you can validate the chaining mechanism on synthetic primitives.
7. **More hunters, in parallel.** Now you're scaling horizontally.
8. **`report-exec` and `report-tech` skills.** Make the output something a human wants to read.
9. **DAST server + PreToolUse gating.** Add only after SAST mode is solid.
10. **Bitbucket pipe wrapping.** Last, because everything else has been runnable ad-hoc all along.

---

## 19. Things to decide before you start coding

A few open questions you should answer up front; they shape several files:

- **Where does the application manifest live?** In the primary repo? A dedicated config repo? Fetched from a registry?
- **How are Bitbucket credentials granted?** App password per workspace, or OAuth consumer? Repo access scope?
- **DAST safety contract.** What's the exact allow-list of destructive verbs? Who signs off on a new target?
- **KG retention.** Do scans pile up? Compare across runs? Diff findings between PRs?
- **Human-in-the-loop touchpoints.** Where does a human get to say "ignore this finding"? Suppressions file in repo?
- **Per-finding cost cap.** "Token budget unlimited" is fine for analysis depth, but you'll still want a *wall-clock* and *spend* cap per scan to avoid runaway loops on a misconfigured manifest.

---

## 20. The Mythos lessons baked in
